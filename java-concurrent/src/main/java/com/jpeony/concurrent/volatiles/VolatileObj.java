...
public class VolatileObj {
    ...
    /**
     * 所有实例共享一个静态变量，不保证多个线程之间数据的一致性，不一定能取到主存中最新的数据，例如 A 线程 new 的对象修改值对 B 线程可见，但是 B 线程不一定能马上看到 A 线程修改的最新值
     */
    public static int static_i = 0;
    /**
     * 所有线程操作同一个实例对象的，保证多个线程之间数据的一致性，一定能取到主存中最新的数据，但是多个实例有多个 volatile 变量，互不相关
     */
