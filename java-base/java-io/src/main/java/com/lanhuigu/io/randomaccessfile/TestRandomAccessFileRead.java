package com.lanhuigu.io.randomaccessfile;

import java.io.File;
import java.io.RandomAccessFile;

public class TestRandomAccessFileRead {
    ...
    public static void main(String[] args) {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(file, "r")) {
            ...
            int hasRead = 0;
            // 循环读取
            while((hasRead=raf.read(readData)) > 0) {
                // 打印读取的内容,并将字节转为字符串输入
                System.out.println(new String(readData,0,hasRead));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
